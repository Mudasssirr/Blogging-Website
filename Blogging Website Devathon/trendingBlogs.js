import { auth, db, getDocs, collection, query, where, orderBy, doc, getDoc, addDoc, deleteDoc, serverTimestamp, updateDoc, increment } from './firebase.js'
...
async function renderBlogs(currentPage, currentUID) {
    ...
    const userBlogsQuery = query(blogRef, where('Popularity', '>=', 10), orderBy('Popularity'));
    const userBlogsSnapshot = await getDocs(userBlogsQuery);
    const totalBlogs = userBlogsSnapshot.docs.length;
    const totalPages = Math.ceil(totalBlogs / itemsPerPage);

    // Add the logic to update Popularity
    userBlogsSnapshot.forEach(async (doc) => {
        const lastUpdated = doc.data().LastPopularityUpdate.toDate();
    ...
    if (userBlogsSnapshot.empty) {
        ...
        allBlogContainer.innerHTML = `
            <div class="no-blogs-message">
                <div class="message-container">
                    ...
                    <p class="no-blogs-subHeading">No trending blogs at the moment. Be the first to set a trend!</p>
                </div>
            </div>
            `;
    } else {
        ...
        reversedBlogs.forEach((doc) => {
            ...
            const HOTPara = (doc.data().Popularity >= 25) ? ', (HOT)' : '';
            ...
            // Build the HTML string for each blog
            blogsHTML += `
                ...
                <div class="col-md-8">
                    ...
                    <div class="card-body">
                        ...
                        <p class="card-text">
                            ...
                            <small> <span class="HOT-p">${HOTPara}</span></small>
                        </p>
    ...
    `
            if (allBlogContainer) {
                allBlogContainer.innerHTML = blogsHTML;
            }
